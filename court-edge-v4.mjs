...
async function buildBoard(league,file,source,events,games,{injuriesStatus='LIMITED',injuryRows=[],liveBoxes=[]}={}){
  ...
  for(const ev of events||[]){const hrs=hoursToStart(ev.commence_time);if(hrs>RADAR_MAX_DAYS*24)continue;const eventId=stableEventId(league,ev),z=buildMarketsForEvent({league,ev:{...ev,id:eventId},model,history,prevMarkets:prevMap,injuries,injuryStatus:injuriesStatus});
    ...
    if(hrs>0){
      ...
      const hp=z.projected.mlProb,forecastReady=z.projected.sample>=20&&z.projected.reliability>=.6,homeScore=(z.projected.projectedTotal+z.projected.projectedMargin)/2,awayScore=(z.projected.projectedTotal-z.projected.projectedMargin)/2;
      ...
      radar.push({event_id:eventId,home_team:ev.home_team,away_team:ev.away_team,start_at:ev.commence_time,hours_to_start:hrs,projected_margin:round(z.projected.projectedMargin,2),projected_total:round(z.projected.projectedTotal,2),projected_home_score:round(homeScore,1),projected_away_score:round(awayScore,1),home_win_prob:forecastReady?round(hp,4):null,forecast_state:forecastReady?'READY':'THIN',projected_winner:forecastReady?(hp>=.5?ev.home_team:ev.away_team):null,projected_winner_prob:forecastReady?round(hp>=.5?hp:1-hp,4):null,model_sample:z.projected.sample,reliability:round(z.projected.reliability,4),margin_sigma:round(z.projected.marginSigma,2),total_sigma:round(z.projected.totalSigma,2),rest:z.projected.rest,team_stats:{home:snapshot(hs),away:snapshot(as)},injuries:z.injuries,status:hrs<=LOCK_MAX_HOURS?'LOCK WINDOW':'EARLY RADAR'});
    }
