(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.CourtQuantMath=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
'use strict';
...
function deVigTwo(oddsA,oddsB){if(!validOdds(oddsA)||!validOdds(oddsB))return null;const ia=1/Number(oddsA),ib=1/Number(oddsB),book=ia+ib;if(!(book>0))return null;return{probA:ia/book,probB:ib/book,hold:book-1}}
function brier(prob,outcome){if(!validProb(prob)||![0,1,false,true].includes(outcome))return null;const y=Number(outcome);return(Math.pow(Number(prob)-y,2))}
...
function priceCushion(currentOdds,minOdds){return validOdds(currentOdds)&&validOdds(minOdds)?Number(currentOdds)/Number(minOdds)-1:null}
return{impliedProbability,fairOdds,expectedValue,probabilityEdge,fullKelly,fractionalKelly,deVigTwo,brier,logLoss,priceCushion};
});
