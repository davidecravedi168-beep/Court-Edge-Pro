...
async function patchEngine(){
  ...
  if(!s.includes(oldEuro)) throw new Error('Euro history target not found');
  s=s.replace(oldEuro,newEuro);
  ...
  if(!s.includes(oldForecast)) throw new Error('Forecast readiness target not found');
  s=s.replace(oldForecast,newForecast);
  s=s.replace("home_win_prob:round(hp,4),projected_winner:hp>=.5?ev.home_team:ev.away_team,projected_winner_prob:round(hp>=.5?hp:1-hp,4),model_sample:z.projected.sample,reliability:round(z.projected.reliability,4)","home_win_prob:forecastReady?round(hp,4):null,forecast_state:forecastReady?'READY':'THIN',projected_winner:forecastReady?(hp>=.5?ev.home_team:ev.away_team):null,projected_winner_prob:forecastReady?round(hp>=.5?hp:1-hp,4):null,model_sample:z.projected.sample,reliability:round(z.projected.reliability,4)");
...
await patchEngine();
await patchIndex();
console.log('Euro history and thin forecast fix applied');
