...
async function patchEngine(){
  ...
  const oldWinner="home_win_prob:round(hp,4),projected_winner:hp>=.5?ev.home_team:ev.away_team,projected_winner_prob:round(hp>=.5?hp:1-hp,4),model_sample:z.projected.sample,reliability:round(z.projected.reliability,4)";
  const newWinner="home_win_prob:forecastReady?round(hp,4):null,forecast_state:forecastReady?'READY':'THIN',projected_winner:forecastReady?(hp>=.5?ev.home_team:ev.away_team):null,projected_winner_prob:forecastReady?round(hp>=.5?hp:1-hp,4):null,model_sample:z.projected.sample,reliability:round(z.projected.reliability,4)";
  if(s.includes(oldWinner)) s=s.replace(oldWinner,newWinner);
  else if(!s.includes("forecast_state:forecastReady?'READY':'THIN'")) throw new Error('Winner suppression target not found');
...
async function patchIndex(){
  ...
  let s=await fs.readFile(file,'utf8');
  if(s.includes('<div class="forecastPick">MODEL LEAN · ${esc(x.projected_winner)}</div>')){
  ...
  if(s.includes('⚠ Pronostico modello anticipato. Senza quota fresca e confronto col mercato resta RESEARCH / NO BET.')){
...
await patchEngine();
await patchIndex();
console.log('EuroLeague final history + thin forecast fix applied');
